...
exports.Client = class {
    ...
    set(key, field, value, callback) {
        ...
        if (field) {
            ...
            return redis.hset(key, field, valueString, (err, created) => process(err, created, { [field]: valueString }));
        }
    ...
    expire(key, ttl, callback) {
        ...
        return this.redis.pexpire(key, ttl, callback);
    }

    increment(key, field, increment, callback) {
        ...
        const script = `if redis.call('hexists', '${key}', '${field}') == 1 then return redis.call('hincrby', '${key}', '${field}', ${increment}) else return nil end`;
        return redis.eval(script, 0, (err, updated) => {
            ...
            return this._publish(redis, key, 'hset', Querystring.stringify({ [field]: updated }), (err) => callback(err, updated));
        });
    }
